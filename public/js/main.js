...
/**
 * Maman-Style Preloader Controller
 * Clean, minimal, elegant loading experience
 */
...
//   // Configuration
//   const config = {
//     minimumLoadTime: 3000, // Minimum time to show preloader
//     fadeOutDuration: 1200, // Fade out transition time
//   }
...
//     targetPercentage = Math.min(Math.floor(progress), 100)
//     animatePercentage()
//   }
...
//     const progress = Math.min((elapsedTime / config.minimumLoadTime) * 100, 95)
...
//   /**
//    * Hide preloader with elegant fade
//    */
//   function hidePreloader() {
//     // Complete the progress bar
//     updateProgress(100)

//     // Wait a moment at 100% before fading
//     setTimeout(() => {
...
//     if (isContentLoaded && elapsedTime >= config.minimumLoadTime) {
//       hidePreloader()
//     } else {
//       const remainingTime = Math.max(0, config.minimumLoadTime - elapsedTime)
...
//         0,
//         config.minimumLoadTime - (Date.now() - startTime)
//       )
...
; (function ($) {
  "use strict"
  // years of experince
  const yearsDiv = document.querySelector(".countup-number")
  if (yearsDiv) {
    const years = 2025 - 2004
    yearsDiv.setAttribute("data-count", years)
  }
  ...
  if ($(".portfolio-filter-box")?.length) {
    $(".portfolio-filter-box").imagesLoaded(function () {
      ...
      //   })
      var $grid = $(".portfolio-filter-box").isotope({
        layoutMode: "fitRows", // Try this instead of masonry
        itemSelector: ".portfolio-filter-box .portfolio-filter-item",
        percentPosition: true,
        fitRows: {
          gutter: 0, // Your gutter size in pixels
        },
      })
...
allMenus.forEach((menu) => {
  const selected = menu.querySelector(".selected-lang")
  const options = menu.querySelectorAll("ul li a")

  // 1. Toggle the clicked menu
  menu.addEventListener("click", (e) => {
    // Prevent the "click outside" code from running immediately
    e.stopPropagation()

    // Optional: Close all other menus before opening this one
    allMenus.forEach((m) => {
  ...
  // 2. Handle option clicks (Scope: only links inside THIS menu)
...
// 3. Close ALL menus when clicking anywhere else on the page
document.addEventListener("click", () => {
  ...
  // Section Navigation - Smooth Scroll and Active State
  ; (function () {
    ...
    if (!sectionNav || !navLinks.length) return

    // Get all sections that have IDs and are linked in nav
    const sections = []
